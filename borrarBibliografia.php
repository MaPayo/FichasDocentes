...
<head>
  ...
  ?>
  <title>Gestión Docente: Bibliografia</title>
</head>
              ...
              <div class="col-md-6 col-12">
                <div class="card">
                  <div class="card-header text-center">
                    <h2>Eliminar el borrador de la bibliografía</h2>
                  </div>
                  <div class="card-body text-center">
                    ...
                    if (isset($_GET['Confirmacion']) && $_GET['Confirmacion'] === 'y') {
                      ...
                      if ($contextBibliografia->getEvent() === FIND_MODBIBLIOGRAFIA_OK) {
                        $context = new es\ucm\Context(DELETE_MODBIBLIOGRAFIA, $IdAsignatura);
                        $contextBibliografia = $controller->action($context);
                        if ($contextBibliografia->getEvent() === DELETE_MODBIBLIOGRAFIA_OK) {
                          header('Location: indexAcceso.php?IdGrado=' . $_GET['IdGrado'] . '&IdAsignatura=' . $IdAsignatura . '&eliminado=y');
                        } elseif ($contextBibliografia->getEvent() === DELETE_MODBIBLIOGRAFIA_FAIL) {
                          header('Location: indexAcceso.php?IdGrado=' . $_GET['IdGrado'] . '&IdAsignatura=' . $IdAsignatura . '&eliminado=n');
                        }
                      }
                    } else {
                    ?>
                      ¿Estás seguro de que quieres eliminar el borrador de la bibliografía?
                      <div class="text-center">
                        <a href="indexAcceso.php?IdGrado=<?php echo $_GET['IdGrado']; ?>&IdAsignatura=<?php echo $IdAsignatura; ?>">
                          <button type="button" class="btn btn-secondary" id="btn-form">
                            Cancelar
                          </button>
                        </a>
                        <a href="borrarBibliografia.php?IdGrado=<?php echo $_GET['IdGrado']; ?>&IdModAsignatura=<?php echo $IdAsignatura; ?>&Confirmacion=y">
