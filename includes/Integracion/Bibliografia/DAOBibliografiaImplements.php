...
class DAOBibliografiaImplements implements DAOBibliografia
{
    ...
    public static function updateBibliografia($bibliografia)
    {
        $singletonDataSource = new SingletonDataSource();
        $dataSource = $singletonDataSource->getInstance();
        $sql = "UPDATE bibliografia SET IdBibliografia = :idBibliografia, CitasBibliograficas = :citasBibliograficas, RecursosInternet = :recursosInternet, IdAsignatura = :idAsignatura WHERE IdAsignatura = :idAsignatura";
        $values = array(
            ':idBibliografia' => $bibliografia->getIdBibliografia(),
            ':citasBibliograficas' => $bibliografia->getCitasBibliograficas(),
            ':recursosInternet' => $bibliografia->getRecursosInternet(),
